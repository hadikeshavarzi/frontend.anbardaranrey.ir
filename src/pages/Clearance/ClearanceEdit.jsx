...
import { useParams, useNavigate } from "react-router-dom"; // تغییر: اضافه شدن useParams و useNavigate
...
import {
    ...
    uploadDeliveryFile,
    getClearanceFullDetails, // تابع دریافت جزئیات
    updateClearanceFull      // تابع آپدیت کامل
} from "../../services/clearanceService";
import { requestOtp, verifyOtp } from "../../services/auth";
import DatePickerWithIcon from "../../components/Shared/DatePickerWithIcon";

// ==========================================
// 1. کامپوننت پلاک (کپی شده برای استفاده داخلی)
// ==========================================
const IranLicensePlate = ({ value, onChange }) => {
    ...
    useEffect(() => {
        if(value) { const p = value.split('-'); setPartLeft(p[0]||""); setPartMid(p[1]||""); setPartLetter(p[2]||"ب"); setPartIran(p[3]||""); }
    }, [value]);

    const updateParent = (left, mid, letter, iran) => onChange(`${left}-${mid}-${letter}-${iran}`);
    ...
    return (
        <div className="d-flex align-items-center border rounded p-1 bg-white shadow-sm" style={{direction: 'ltr', maxWidth: '280px', height: '45px'}}>
            <div className="bg-primary text-white d-flex flex-column align-items-center justify-content-center rounded-start h-100 px-1" style={{fontSize: '9px', width: '35px'}}><span>I.R.</span><span>IRAN</span></div>
            ...
            <Input type="select" className="border-0 text-center fw-bold p-0 text-danger font-size-14" style={{width: '55px', boxShadow: 'none', cursor: 'pointer'}} value={partLetter} onChange={(e) => { const v = e.target.value; setPartLetter(v); updateParent(partLeft, partMid, v, partIran); }}>{letters.map(l => <option key={l} value={l}>{l}</option>)}</Input>
...
// ==========================================
// 2. کامپوننت‌های درختی و جدول (Recursive)
// ==========================================
const buildHistoryTree = (flatHistory, rootBatchNo) => {
    const tree = [];
    const directChildren = flatHistory.filter(h => h.parent_batch_no === rootBatchNo);
    directChildren.forEach(child => { tree.push({ ...child, children: buildHistoryTree(flatHistory, child.batch_no) }); });
    return tree;
};
...
const RecursiveRow = ({ item, level, onOpenModal, flatHistory }) => {
    ...
    const getNodeStats = () => {
        if (level === 0) return { qty: item.qty_available, weight: item.weight_available };
        else {
            const initialQty = Number(item.qty || 0); const initialWeight = Number(item.weight || 0);
            ...
            return { qty: initialQty - usedQty, weight: initialWeight - usedWeight };
        }
    };
    const handleBreakDown = () => {
        ...
        // در حالت ویرایش، ممکن است موجودی 0 باشد اما چون خودمان مصرف کردیم، سخت‌گیری نمی‌کنیم (فقط هشدار)
        if (stats.qty <= 0 && stats.weight <= 0) { /* toast.warn("موجودی تمام شده"); */ }
        ...
        onOpenModal({ batch_no: currentBatchName, qty_available: stats.qty, weight_available: stats.weight, next_index: nextIndex, isNoBatch: isNoBatch });
    };
    const rowStyle = { backgroundColor: level === 0 ? "#fff" : `rgba(240, 242, 245, ${0.3 + (level * 0.05)})`, borderRight: level > 0 ? `4px solid ${level % 2 === 0 ? '#34c38f' : '#556ee6'}` : 'none' };
    return (
        <>
            <tr style={rowStyle} className="align-middle">
                <td className="ps-3">
                    <div className="d-flex align-items-center" style={{ marginLeft: `${level * 25}px` }}>
                        ...
                        <span className={`ms-2 font-monospace ${level===0 ? 'fw-bold text-primary font-size-14' : 'font-size-13 text-dark'}`}>{(item.batch_no || item.new_batch_no) || "بدون ردیف"}</span>
                    </div>
                </td>
                {level === 0 ? (
                    <>
                ...
                <td className="text-end pe-3">
                    <div className="d-flex justify-content-end gap-2 align-items-center">
                        <Button color="primary" size="sm" className="btn-rounded py-1 px-3 shadow-sm" onClick={handleBreakDown}><i className="bx bx-git-branch me-1"></i> <span>خرد کردن / خروج</span></Button>
                        {level > 0 && item.attachment_url && (<a href={item.attachment_url} target="_blank" rel="noreferrer" className="btn btn-sm btn-soft-secondary rounded-circle ms-1"><i className="bx bx-download"></i></a>)}
                    </div>
                </td>
            </tr>
            {isOpen && hasChildren && item.children.map(child => <RecursiveRow key={child.id} item={child} level={level + 1} onOpenModal={onOpenModal} flatHistory={flatHistory} />)}
        </>
    );
};
...
export default function ClearanceEdit() {
    ...
    const [items, setItems] = useState([]); // لیست اقلام سند
    const [productList, setProductList] = useState([]);
    const [batchData, setBatchData] = useState([]);

    // مودال‌ها
    const [modalOpen, setModalOpen] = useState(false); // مودال افزودن آیتم
    ...
    // 1. دریافت لیست صاحبان کالا (اولیه)
    useEffect(() => {
    ...
    // 2. لود اطلاعات سند برای ویرایش
    useEffect(() => {
        if (id && customerOptions.length > 0) {
            const loadEditData = async () => {
                try {
                    const details = await getClearanceFullDetails(id);

                    // پر کردن هدر فرم
                    setValue("docDate", new Date(details.clearance_date));
                    setValue("customerId", details.customer_id);
                    setValue("manualDocRef", details.description ? details.description.replace("سند: ", "") : "");

                    // ساختن رشته پلاک از اجزای آن
                    const fullPlate = `${details.vehicle_plate_left2 || ''}-${details.vehicle_plate_mid3 || ''}-${details.vehicle_plate_letter || 'ب'}-${details.vehicle_plate_iran_right || ''}`;

                    // پر کردن جدول آیتم‌ها
                    const mappedItems = details.items.map(i => ({
                        id: i.id, // آیدی دیتابیس
                        product: { id: i.product_id, title: i.product.name },
                        parentBatch: i.parent_batch_no || "بدون ردیف",
                        newBatch: i.new_batch_no || "بدون ردیف",
                        qty: Number(i.qty),
                        weight: Number(i.weight),
                        manualRef: i.manual_ref_id,
                        fileUrl: i.attachment_url,
                        status: i.status,
                        receiverDetails: {
                            name: details.receiver_person_name,
                            nationalId: details.receiver_person_national_id,
                    ...
                    // لود محصولات این مشتری
                    fetchProducts(details.customer_id);

                } catch (err) {
                    toast.error("خطا در بارگذاری اطلاعات: " + err.message);
                    navigate("/clearances/report");
                }
            };
            loadEditData();
        }
    }, [id, customerOptions, setValue, navigate]);

    // 3. لود محصولات وقتی مشتری تغییر میکند
    useEffect(() => {
    ...
    const handleOpenModal = (batchNode) => {
        ...
        const defaultReceiver = items.length > 0 ? items[0].receiverDetails : {};
        setModalInput({
            qty: "", weight: "", manualRef: "",
            receiverName: defaultReceiver.name || "",
            receiverNationalId: defaultReceiver.nationalId || "",
            driverName: defaultReceiver.driver || "",
            plateNumber: defaultReceiver.plate || "",
            description: ""
        });
    ...
    const generatedNewBatch = useMemo(() => {
        ...
        if(selectedBatchNode.isNoBatch || selectedBatchNode.batch_no === "بدون ردیف") return "بدون ردیف";
    ...
    const handleAddToTable = async () => {
        ...
        try {
            ...
            if (finalRemainingQty > 0) {
                ...
                else if (inputQty > finalRemainingQty) {
                    // در حالت ویرایش هشدار میدهیم ولی بلاک نمیکنیم (چون موجودی ممکن است برای خود این سند باشد)
                    // errors.qty = `حداکثر ${finalRemainingQty}`; hasError = true;
                }
            }
            ...
            if (selectedFile) {
                ...
                catch(err) { toast.error("خطا در آپلود فایل"); setUploading(false); return; }
                setUploading(false);
            }
        ...
        } catch (error) { toast.error("خطا در افزودن"); }
    };
    ...
    const productOptions = useMemo(() => {
        return productList.map(p => ({
            ...
            customData: { title: p.product_title, qty: Number(p.total_qty_available || 0), weight: Number(p.total_weight_available || 0) },
    ...
    const handleUpdateSubmit = (status) => {
        const onValid = async (headerData) => {
            ...
            try {
                ...
                if (!selectedCustomer || !selectedCustomer.mobile) { toast.error("شماره موبایل یافت نشد"); setSaving(false); return; }
        ...
        handleSubmit(onValid, (errors) => { toast.warning("لطفا فرم را تکمیل کنید"); })();
    };
    ...
    const handleVerifyAndUpdate = async () => {
        ...
        try {
            ...
            if (verifyRes.success || verifyRes.token) {
                // *** فراخوانی تابع آپدیت کامل ***
                await updateClearanceFull(id, pendingPayload);
                toast.success("سند با موفقیت ویرایش شد");
                navigate("/clearances/report"); // بازگشت به لیست
    ...
    return (
        <div className="page-content" style={{fontFamily: 'inherit'}}>
            <Container fluid>
                <Card className="shadow-sm border-0 mb-4 bg-soft-warning border-warning">
                    <CardBody className="p-4">
                        <div className="d-flex align-items-center justify-content-between mb-4 border-bottom pb-3">
                            <CardTitle className="h4 text-warning mb-0"><i className="bx bx-edit me-2"></i> ویرایش سند خروج</CardTitle>
                            <Badge color="warning" className="text-dark font-size-13 p-2">شماره سند: {id}</Badge>
                        </div>

                        {/* فرم ورودی هدر */}
                        <div className="bg-white p-4 rounded-3 mb-4 border border-dashed">
                            <Row>
                                <Col md={3}>
                                ...
                                <Col md={6}>
                                    <Label>صاحب کالا (غیرقابل تغییر در ویرایش)</Label>
                                    <Controller name="customerId" control={control} render={({ field }) => (
                                        <Select {...field} options={customerOptions} placeholder="در حال بارگذاری..." value={customerOptions.find(c => c.value === field.value)} onChange={val => field.onChange(val ? val.value : null)} isDisabled />
                                    )} />
                                </Col>
                        ...
                        {/* بخش انتخاب محصول و افزودن آیتم */}
                        <div className="mb-5">
                            <Row className="mb-3">
                                <Col md={12}>
                                    <Label>افزودن/تغییر کالا</Label>
                                    <Select
                                        value={selectedProductId}
                                        onChange={handleProductChange}
                                        options={productOptions}
                                        isDisabled={!selectedCustomerId}
                                        placeholder="برای تغییر یا افزودن کالا جستجو کنید..."
                                    />
                                </Col>
                            </Row>
                            {batchData.length > 0 && (
                                <div className="table-responsive rounded border shadow-sm bg-white">
                        ...
                        {items.length > 0 && (
                            <div className="mb-3 animate__animated animate__fadeInUp">
                                <h6 className="font-size-14 text-secondary border-start border-3 border-warning ps-2 mb-3">اقلام سند (قابل ویرایش)</h6>
                                <Table className="table align-middle table-nowrap table-striped border mb-0 bg-white">
                        ...
                        <div className="d-flex justify-content-end gap-2 border-top pt-4 mt-4">
                            <Button color="secondary" size="lg" onClick={() => navigate("/clearances/report")}>انصراف</Button>
                            <Button color="warning" size="lg" className="px-5 shadow" onClick={()=>handleUpdateSubmit('final')} disabled={saving || items.length===0}>
                                {saving ? "در حال پردازش..." : "ذخیره تغییرات"}
                            </Button>
                        </div>
                    </CardBody>
                </Card>
                ...
                <Modal isOpen={modalOpen} toggle={() => setModalOpen(false)} size="lg" centered>
                    <ModalHeader toggle={() => setModalOpen(false)} className="bg-light"><span className="fw-bold">خروج از:</span> <span className="font-monospace text-primary bg-white px-2 py-1 rounded border ms-2">{selectedBatchNode?.batch_no}</span></ModalHeader>
                    <ModalBody className="p-4">
                        ...
                        <Row>
                            <Col md={6} className="mb-3">
                                <Label>تعداد</Label>
                                <InputGroup><InputGroupText className="bg-white"><i className="bx bx-hash"></i></InputGroupText><Input type="number" value={modalInput.qty} onChange={e=> { setModalInput({...modalInput, qty:e.target.value}); if(fieldErrors.qty) setFieldErrors({...fieldErrors, qty: null}); }} invalid={!!fieldErrors.qty} autoFocus /><FormFeedback>{fieldErrors.qty}</FormFeedback></InputGroup>
                            </Col>
                            <Col md={6} className="mb-3">
                                <Label>وزن</Label>
                                <InputGroup><InputGroupText className="bg-white"><i className="bx bx-dumbbell"></i></InputGroupText><Input type="number" value={modalInput.weight} onChange={e=> { setModalInput({...modalInput, weight:e.target.value}); if(fieldErrors.weight) setFieldErrors({...fieldErrors, weight: null}); }} invalid={!!fieldErrors.weight} /><FormFeedback>{fieldErrors.weight}</FormFeedback></InputGroup>
                            </Col>
                        </Row>
                        <hr className="my-2 border-light" />
                        <Row>
                            <Col md={6} className="mb-3"><Label>گیرنده <span className="text-danger">*</span></Label><Input value={modalInput.receiverName} onChange={e=> { setModalInput({...modalInput, receiverName:e.target.value}); if(fieldErrors.receiverName) setFieldErrors({...fieldErrors, receiverName: null}); }} invalid={!!fieldErrors.receiverName} placeholder="مثال: آقای حسینی" /><FormFeedback>{fieldErrors.receiverName}</FormFeedback></Col>
                    ...
                    <ModalFooter><Button color="link" onClick={()=>setModalOpen(false)}>انصراف</Button><Button color="success" onClick={handleAddToTable} disabled={uploading}>{uploading ? "آپلود..." : "افزودن"}</Button></ModalFooter>
                </Modal>
                ...
                <Modal isOpen={otpModalOpen} toggle={() => setOtpModalOpen(false)} size="sm" centered>
                    ...
                    <ModalBody className="text-center p-4">
                        ...
                        <Button color="warning" className="w-100 btn-lg shadow" onClick={handleVerifyAndUpdate} disabled={saving}>{saving ? "بررسی..." : "تایید ویرایش"}</Button>
                    </ModalBody>
                </Modal>
