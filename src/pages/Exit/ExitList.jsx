...
import Swal from "sweetalert2"; // برای تایید حذف خوشگل‌تر است (اگر ندارید window.confirm استفاده کنید)
    ...
    useEffect(() => {
        if (!searchTerm) {
            ...
            const filtered = data.filter(item => 
                ...
                item.loading_orders?.driver_name?.includes(searchTerm) ||
                item.loading_orders?.plate_number?.includes(searchTerm) ||
                item.loading_orders?.clearances?.customers?.name?.includes(searchTerm)
            );
                            ...
                            <Col md={4}>
                                <div className="search-box position-relative">
                                    <Input
                                        ...
                                        placeholder="جستجو (راننده، پلاک، شناسه...)"
                                        value={searchTerm}
                                    ...
                                    {loading ? (
                                        ...
                                        filteredData.map((item) => (
                                            <tr key={item.id}>
                                                ...
                                                <td>
                                                    <h5 className="font-size-13 mb-1">{item.loading_orders?.driver_name}</h5>
                                                    <p className="text-muted mb-0 font-size-12" dir="ltr">{item.loading_orders?.plate_number}</p>
                                                </td>
                                                <td>{item.loading_orders?.clearances?.customers?.name || "---"}</td>
                                                ...
                                                <td className="text-center">
                                                    <div className="d-flex justify-content-center gap-2">
                                                        ...
                                                        <Button 
                                                            ...
                                                            onClick={() => navigate(`/exit/create?orderNo=${item.loading_orders.order_no}`)}
                                                            id={`edit-${item.id}`}
                                                        >
